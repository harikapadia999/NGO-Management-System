...
mongoose.connect(
  "mongodb://127.0.0.1:27017/ngosysdb",
  {
...
app.post("/registervol", (req, res) => {
  ...
  Volunteer.findOne({ email: email }, (err, user) => {
    if (user) {
      res.send({
        message: "User Already Registered. Please use another Email Id.",
      });
    } else {
...
app.get("/thingsRegister", (req, res) => {
    const { donorName, address, quantity, phoneNumber, Image } = req.body;
  
    // Donor.findOne({ phoneNumber: phoneNumber }, (err, existingDonor) => {
      if (err) {
        ...
        donor.save((err) => {
          if (err) {
            res.status(500).send({ error: "Error while saving donor information" });
          } else {
...
app.post("/registerngo", (req, res) => {
  const { name, address, city, state, NGOID, pnumber, email, passwd } =
    req.body;
...
app.post("/updatengo", (req, res) => {
  ...
  NGO.findOneAndUpdate(
    ...
    (err, user) => {
      ...
      res
        .status(200)
        .json({ user, message: "Ngo Details Updated Successfully." });
    }
  );
});
...
app.post("/updatevol", (req, res) => {
  ...
  Volunteer.findOneAndUpdate(
    ...
    (err, user) => {
      ...
      res
        .status(200)
        .json({ user, message: "Volunteer Details Updated Successfully. " });
    }
  );
});
...
app.post("/updatedon", (req, res) => {
  ...
  Donor.findOneAndUpdate(
    ...
    (err, user) => {
      ...
      res
        .status(200)
        .json({ user, message: "Donor Details Updated Successfully. " });
    }
  );
});
  ...
  if (term) {
    Events.find({
      ...
      .exec((err, events) => {
        ...
        res
          .status(200)
          .json({ success: true, events, Postsize: events.length });
      });
  } else {
    Events.find({ $and: [findArgs, { edate: { $gt: today.toISOString() } }] })
...
app.post("/addToCart/:id/:id1", (req, res) => {
  ...
  Volunteer.findById(req.params.id1).then((user) => {
...
app.post("/removeregevent/:eid/:volid", (req, res) => {
  Volunteer.findById(req.params.volid).then((userInfo) => {
    Volunteer.findOneAndUpdate(
      { _id: req.params.volid },
      {
        $pull: { cart: { id: req.params.eid } },
      },
      { new: true },
      (err, user) => {
        let cart = user.cart;
        ...
        Events.findOneAndUpdate(
          { _id: req.params.eid },
          {
            $pull: { participants: { id: req.params.volid } },
          },
          { new: true },
            ...
            Events.find({ _id: { $in: array } })
              ...
              .exec((err, cart) => {
                return res.status(200).json({
                  ...
                  cart,
                  message:
                    "Registration canceled successfully, please reload. ",
                });
              });
          }
        );
...
app.post("/updateevents", (req, res) => {
  ...
  Events.findOneAndUpdate(
    ...
    (err, user) => {
      ...
      res
        .status(200)
        .json({ user, message: "Event details Updated Successfully." });
    }
  );
});
...
app.post("/ngoevents/:ngoid", (req, res) => {
  ...
  Events.find({ ngoid: req.params.ngoid }).then;
});
  ...
  Events.findById(req.params.eventid).then((event) => {
    Events.findOneAndUpdate(
      ...
      (err, event) => {
        ...
        res
          .status(200)
          .json({ event, message: "Feedback Submitted Successfully." });
      }
    );
  });
});
...
app.post("/removefeedback/:eventId", (req, res) => {
  Events.findById(req.params.eventId).then((userInfo) => {
    Events.findOneAndUpdate(
      { _id: req.params.eventId },
      {
        $pull: {
        ...
        Events.find({ id: { $in: array } })
          ...
          .exec((err, feedback) => {
            return res.status(200).json({
              user,
              feedback,
              mess: "Feedback Removed Successfully, please refresh to view the changes.",
            });
          });
      }
    );
  ...
  Events.findById(req.params.eventid).then((event) => {
    Events.findOneAndUpdate(
      ...
      (err, event) => {
        ...
        res
          .status(200)
          .json({ event, message: "Suggestion Submitted Successfully. " });
      }
    );
  });
});
...
app.get("/transactions/:ngoID", async (req, res) => {
  ...
  try {
    ...
    const transactions = await Transection.find({ ngoid: ID })
      .populate("donorid")
      .populate("ngoid");
